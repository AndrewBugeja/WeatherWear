...
public class LocationService {
    ...
    public LocationIdentifier primaryGetMyLoc() throws IOException, InterruptedException {

        HttpResponse<String> response = null;
        try {
            HttpRequest request = HttpRequest.newBuilder()
                    .uri(URI.create("https://weatherapi-com.p.rapidapi.com/ip.json?q=auto%3Aip"))
                    .timeout(Duration.ofSeconds(3)).header("X-RapidAPI-Key", weatherApiKey)
          ...
          response = HttpClient.newHttpClient().send(request, HttpResponse.BodyHandlers.ofString());
            ...
            HttpRequest request = HttpRequest.newBuilder().uri(URI.create("http://ip-api.com/json/24.48.0.1?fields=lat,lon"))
                    .method("GET", HttpRequest.BodyPublishers.noBody()).build();
            response = HttpClient.newHttpClient().send(request, HttpResponse.BodyHandlers.ofString());
        }
    ...
    public LocationIdentifier parseLoc(HttpResponse<String> response) throws HttpTimeoutException {

        if(response == null){
            throw new HttpTimeoutException("Something went wrong with the location service...");
        }
        LocationIdentifier loc = new LocationIdentifier();
    ...
    public String getFutureLocation(String location, String dt) throws Exception{
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("https://weatherapi-com.p.rapidapi.com/forecast.json?q=metar%3A"+location+"&dt="+dt))
                .header("X-RapidAPI-Key", weatherApiKey).header("X-RapidAPI-Host", "weatherapi-com.p.rapidapi.com")
                .method("GET", HttpRequest.BodyPublishers.noBody()).build();
